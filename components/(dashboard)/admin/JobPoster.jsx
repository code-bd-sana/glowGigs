...
export default function JobPoster() {
  ...
  const [updateStatus, { isLoading: UpdateLoading, error: UpdateError }] =
    useUpdateStatusMutation();
  const [deleteUser, { isLoading: deleteLoading, error: UpdateDeleteError }] =
  ...
  const handleUpdateStatus = async () => {
    if (posterToEdit) {
      ...
      // Show success message
      toast.success(
        `Status updated to ${newStatus} for ${posterToEdit.fullName}!`,
        {
          position: "top-right",
          autoClose: 3000,
        },
